...
export const createMood = async (req, res) => {
    try {
        ...
        const result = await createMoodService(userId, mood, note);
        res.status(201).json({
...
export const getMoods = async (req, res) => {
    const userId = req.user.id;
    try {
        const result = await getMoodService(userId);
        res.status(200).json({
            moods: result
        });
    }
...
export const updateMood = async (req, res) => {
    ...
    try {
        const result = await updateMoodService(moodId, userId, mood, note);
        res.status(200).json({
            moods: result
        });
    }
...
export const deleteMood = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        const result = await deleteMoodService(moodId, userId);
        res.status(200).json({
