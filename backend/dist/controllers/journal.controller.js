...
export const createJournal = async (req, res) => {
    ...
    try {
        const result = await createJournalService(title, content, userId);
        res.status(200).json({
...
export const getJournals = async (req, res) => {
    const userId = req.user.id;
    try {
        const results = await getJournalService(userId);
        res.status(200).json({
...
export const updateJournal = async (req, res) => {
    ...
    try {
        const results = await updateJournalService(title, content, userId, journalId);
        res.status(200).json({
...
export const deleteJournal = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        const results = await deleteJournalService(journalId, userId);
        res.status(200).json({
